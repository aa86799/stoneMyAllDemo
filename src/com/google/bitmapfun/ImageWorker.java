 ...
 * 
 * ImageWorker是一个抽象类,loadImage()方法为外部提供了一个加载图片的统一接口，其中具体的加载过程通过一个异步类来后台执行，
 * 异步类又会调用processBitmap()方法。在这里ImageWork没有实现该方法，因为图片的加载方式有多种，包括网络下载，
 * 从resources中解析和本地文件解析等，因此把它定义为一个抽象方法，具体的实现由子类来决定（也就是传说中的工厂模式）。
 * 在该类中还包含了缓存管理ImageCache的引用，内部类CacheAsyncTask异步调用closeCache等方法管理硬盘缓存。
 */
public abstract class ImageWorker {
    ...
    public static void cancelWork(ImageView imageView) {
        ...
        if (bitmapWorkerTask != null) {
            ...
            if (Constants.DEBUG) {
                ...
                LogUtils.printInfo(TAG, "cancelWork - cancelled work for " + bitmapData);
            }
        }
    }
    ...
    public static boolean cancelPotentialWork(Object data, ImageView imageView) {
        ...
        if (bitmapWorkerTask != null) {
            ...
            if (bitmapData == null || !bitmapData.equals(data)) {
                bitmapWorkerTask.cancel(true);
                if (Constants.DEBUG) {
                    LogUtils.printInfo(TAG, "cancelPotentialWork - cancelled work for " + data);
                }
            } else {
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Object... params) {
            if (Constants.DEBUG) {
                LogUtils.printInfo(TAG, "doInBackground - starting work");
            }
            ...
            if (Constants.DEBUG) {
                LogUtils.printInfo(TAG, "doInBackground - finished work");
            }
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            if (bitmap != null && imageView != null) {
                if (Constants.DEBUG) {
                    LogUtils.printInfo(TAG, "onPostExecute - setting bitmap");
                }
